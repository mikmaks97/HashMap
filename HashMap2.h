...
bool HashMap<T>::set(const std::string key, T value) {
  ...
  do {
    ...
    for (typename std::list<std::pair<std::string, T> >::iterator itPair = listAtKey->begin(); itPair != listAtKey->end(); ++itPair) {
      if (itPair->first == key) //if the key already exists return false
        return false;
    }
...
/*
removes the value stored in map at the position corresponding to the given key and deletes the key-value pair from map
@Precondition key is not empty
@Postcondition key-value pair is deleted from map or map remains unchanged
@parameters the key for the object
@return the value for the given key or the default value for the type of objects stored in map
*/
template<class T>
  ...
  for (int i = 0; i <= 6; i++) {
    ...
    for (typename std::list<std::pair<std::string, T> >::iterator itPair = listAtKey->begin(); itPair != listAtKey->end(); ++itPair) {
      if (itPair->first == key) {
        ...
        listAtKey->erase(itPair); //delete the key-value pair
        if (listAtKey->size() == 0)
          itemsInMap--;  //if the list is now empty, the map has a new empty slot, so decrement number of items
